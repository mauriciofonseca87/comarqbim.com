<?php
namespace DieFinnhutteCore\CPT\Shortcodes\Banner;
...
class Banner implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Banner', 'diefinnhutte-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'diefinnhutte-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'diefinnhutte-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'diefinnhutte-core' )   => 'qodef-visible-on-hover',
								esc_html__( 'Visible on Default', 'diefinnhutte-core' ) => 'qodef-visible-on-default',
								esc_html__( 'Disabled', 'diefinnhutte-core' )           => 'qodef-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'diefinnhutte-core' ),
							'value'       => array(
								esc_html__( 'Default', 'diefinnhutte-core' )  => 'default',
								esc_html__( 'Centered', 'diefinnhutte-core' ) => 'centered',
						...
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'diefinnhutte-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'diefinnhutte-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'title',
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
							'heading'    => esc_html__( 'Subtitle', 'diefinnhutte-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'subtitle_tag',
							'heading'     => esc_html__( 'Subtitle Tag', 'diefinnhutte-core' ),
							'value'       => array_flip( diefinnhutte_select_get_title_tag( true, array( 'p' => 'p' ) ) ),
							'save_always' => true,
						...
						array(
							...
							'heading'    => esc_html__( 'Subtitle Color', 'diefinnhutte-core' ),
							'dependency' => array( 'element' => 'subtitle', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text', 'diefinnhutte-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'link_color',
							'heading'    => esc_html__( 'Link Text Color', 'diefinnhutte-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'diefinnhutte-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		$args   = array(
			...
			'image'                => '',
			'overlay_color'        => '',
			'hover_behavior'       => 'qodef-visible-on-default',
		...
		$html = diefinnhutte_core_get_shortcode_module_template_part( 'templates/banner', 'banner', '', $params );
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . diefinnhutte_select_filter_px( $params['link_top_margin'] ) . 'px';
		}
