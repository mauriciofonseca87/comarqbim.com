<?php
namespace DieFinnhutteCore\CPT\Shortcodes\CallToAction;
...
class CallToAction implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Call To Action', 'diefinnhutte-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Set Content In Grid', 'diefinnhutte-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Content Elements Proportion', 'diefinnhutte-core' ),
							'value'      => array(
								esc_html__( '80/20', 'diefinnhutte-core' ) => '80',
								esc_html__( '75/25', 'diefinnhutte-core' ) => '75',
								esc_html__( '66/33', 'diefinnhutte-core' ) => '66',
								esc_html__( '50/50', 'diefinnhutte-core' ) => '50'
							),
							'dependency' => array( 'element' => 'layout', 'value' => array( 'normal' ) )
						),
						array(
						...
						array(
							...
							'heading'    => esc_html__( 'Button Top Margin (px)', 'diefinnhutte-core' ),
						...
						array(
							...
							'value'      => esc_html__( 'I am test text for Call to Action shortcode content', 'diefinnhutte-core' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$html = diefinnhutte_core_get_shortcode_module_template_part( 'templates/call-to-action', 'call-to-action', '', $params );
	...
	private function getButtonHolderStyles( $params ) {
		...
		if ( ! empty( $params['button_top_margin'] ) && $params['layout'] === 'simple' ) {
			$styles[] = 'margin-top: ' . diefinnhutte_select_filter_px( $params['button_top_margin'] ) . 'px';
		}
