...
class DieFinnhutteSelectClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_diefinnhutte_select_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_diefinnhutte_select_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'diefinnhutte' );
		
		if ( isset( $_COOKIE[ 'qodef-like_' . $post->ID ] ) ) {
			$class = 'qodef-like liked';
			$title = esc_html__( 'You already like this!', 'diefinnhutte' );
		}
...
if ( ! function_exists( 'diefinnhutte_select_create_like' ) ) {
	function diefinnhutte_select_create_like() {
		DieFinnhutteSelectClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'diefinnhutte_select_create_like' );
}