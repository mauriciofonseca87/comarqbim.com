...
foreach ( glob( SELECT_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( diefinnhutte_select_is_customizer_item_enabled( $module_load, 'diefinnhutte_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'diefinnhutte_select_get_icon_pack_collections' ) ) {
	function diefinnhutte_select_get_icon_pack_collections() {
		$options = apply_filters( 'diefinnhutte_select_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'diefinnhutte_select_get_icon_pack_collections' );
}

if ( ! function_exists( 'diefinnhutte_select_get_icon_pack_options' ) ) {
	function diefinnhutte_select_get_icon_pack_options() {
		$options = apply_filters( 'diefinnhutte_select_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'diefinnhutte_select_get_icon_pack_options' );
}

if ( ! function_exists( 'diefinnhutte_select_get_icon_pack_social_options' ) ) {
	function diefinnhutte_select_get_icon_pack_social_options() {
		$options = apply_filters( 'diefinnhutte_select_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'diefinnhutte_select_get_icon_pack_social_options' );
}

/*
 * Class: DieFinnhutteSelectClassIconCollections
 */
class DieFinnhutteSelectClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = diefinnhutte_select_get_icon_pack_collections();
		$this->iconPackOptions       = diefinnhutte_select_get_icon_pack_options();
		$this->iconPackSocialOptions = diefinnhutte_select_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = diefinnhutte_select_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'diefinnhutte' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'diefinnhutte' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'diefinnhutte' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'diefinnhutte' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'diefinnhutte' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'diefinnhutte' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'diefinnhutte' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'diefinnhutte' )       => 'fa-lg',
			esc_html__( 'Small', 'diefinnhutte' )      => 'fa-2x',
			esc_html__( 'Medium', 'diefinnhutte' )     => 'fa-3x',
			esc_html__( 'Large', 'diefinnhutte' )      => 'fa-4x',
			esc_html__( 'Very Large', 'diefinnhutte' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iDieFinnhutteSelectInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'diefinnhutte' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'diefinnhutte' ) ] = '';
